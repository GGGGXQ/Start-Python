from typing import TypeVar, List, Optional, Dict, Tuple
    ...
    while not pq.empty:
        ...
        for we in wg.edges_for_index(u):
            ...
            if dist_v is None or dist_v > we.weight + dist_u:
                ...
                path_dict[we.v] = we
                pq.push(DijkstraNode(we.v, dist_u + we.weight))
    return distances, path_dict


def distance_array_to_vertex_dict(wg: WeightedGraph[V], distances: List[Optional[float]]) -> Dict[V, Optional[float]]:
    distances_dict: Dict[V, Optional[float]] = {}
    for i in range(len(distances)):
        distances_dict[wg.vertex_at(i)] = distances[i]
...
if __name__ == '__main__':
    ...
    name_distance: Dict[V, Optional[int]] = distance_array_to_vertex_dict(city_graph2, distances)
