...
class Network:
    def __init__(self, layer_structure: List[int], learning_rate: float,
        ...
        if len(layer_structure) < 3:
            raise ValueError('Network structure must be at least 3 layers')
    ...
    def back_propagate(self, expected: List[float]) -> None:
        ...
        for last in range(last_layer - 1, 0, -1):
            self.layers[last].calculate_deltas_for_hidden_layer(self.layers[last + 1])
