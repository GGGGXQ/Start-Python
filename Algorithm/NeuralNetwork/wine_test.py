...
if __name__ == '__main__':
    ...
    wine_classification: List[List[float]] = []
    ...
    with open('wine.csv', mode='r') as wine_file:
        ...
        for wine in wines:
            ...
            if species == 1:
                wine_classification.append([1.0, 0.0, 0.0])
            elif species == 2:
                wine_classification.append([0.0, 1.0, 0.0])
            else:
                wine_classification.append([0.0, 0.0, 1.0])
            wine_species.append(species)
    normalize_by_feature_scaling(wine_parameters)
    wine_network: Network = Network([13, 7, 3], 0.9)
    wine_trains: List[List[float]] = wine_parameters[0:150]
    wine_trains_correct: List[List[float]] = wine_classification[0:150]
    for _ in range(10):
        wine_network.train(wine_trains, wine_trains_correct)
    wine_testers: List[List[float]] = wine_parameters[150:]
    wine_tests_correct: List[int] = wine_species[150:]
    wine_results = wine_network.validate(wine_testers, wine_tests_correct, wine_interpret_output)
