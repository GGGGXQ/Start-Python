...
vt_distance: Dict[str, Dict[str, int]] = {
    "Rutland":
        {"Burlington": 67,
...
vt_cities: Iterable[str] = vt_distance.keys()
city_permutations: Iterable[Tuple[str, ...]] = permutations(vt_cities)
tsp_path: List[Tuple[str, ...]] = [c + (c[0], ) for c in city_permutations]


if __name__ == '__main__':
    best_path: Tuple[str, ...] = ()
    min_distance: int = 9999999999
    for path in tsp_path:
        distance: int = 0
        last: str = path[0]
        for next in path[1:]:
            distance += vt_distance[last][next]
            last = next
